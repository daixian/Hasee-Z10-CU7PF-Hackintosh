 ...
 *
 * Disassembly of SSDT-2-PegSsdt.aml, Tue Jul 14 15:50:27 2020
 *
 * Original Table Header:
 *     Signature        "SSDT"
 *     Length           0x00002969 (10601)
 *     Revision         0x02
 *     Checksum         0x6F
 *     OEM ID           "PegSsd"
...
DefinitionBlock ("", "SSDT", 2, "PegSsd", "PegSsdt", 0x00001000)
{
    ...
    External (_SB_.PCI0.EECP, FieldUnitObj)
    ...
    External (_SB_.PCI0.PD02, MethodObj)    // 0 Arguments
    External (_SB_.PCI0.PD0A, MethodObj)    // 0 Arguments
    External (_SB_.PCI0.PD0B, MethodObj)    // 0 Arguments
    External (_SB_.PCI0.PEG0, DeviceObj)    // (from opcode)
    External (_SB_.PCI0.PEG0.CEDR, FieldUnitObj)    // (from opcode)
    External (_SB_.PCI0.PEG0.CMDR, FieldUnitObj)    // (from opcode)
    External (_SB_.PCI0.PEG0.D0ST, FieldUnitObj)    // (from opcode)
    External (_SB_.PCI0.PEG0.LREN, FieldUnitObj)    // (from opcode)
    External (_SB_.PCI0.PEG0.PEGP, DeviceObj)    // (from opcode)
    External (_SB_.PCI0.PEG0.PEGP.HDAE, FieldUnitObj)    // (from opcode)
    External (_SB_.PCI0.PEG0.PEGP.LTRE, IntObj)    // (from opcode)
    External (_SB_.PCI0.PEG0.PEGP.SSSV, FieldUnitObj)    // (from opcode)
                ...
                If (LGreater (OSYS, 0x07D9))
                {
                    If (LEqual (\_SB.PCI0.DGPV, 0x10DE))
                    {
                        ...
                        Else
                        {
                            ...
                            Store (\_SB.PCI0.DSSV, \_SB.PCI0.PEG0.PEGP.SSSV)
                            If (LEqual (\_SB.PCI0.NVGE, 0x11))
                            {
                                If (\_SB.PCI0.OPTF)
                                {
                                    Store (One, \_SB.PCI0.PEG0.PEGP.HDAE)
                                }
                                Else
                                {
                                    Store (Zero, \_SB.PCI0.PEG0.PEGP.HDAE)
                                }
                            }
...
//        })
    ...
    Scope (\_SB.PCI0)
    {
        ...
        Method (PGON, 1, Serialized)
        {
            ...
            If (LEqual (PION, Zero))
            {
                If (LNotEqual (\_SB.PCI0.DGPV, 0x10DE))
                {
                    Store (H0VI, S0VI)
                    Store (H0DI, S0DI)
                }
        ...
        Method (GPPR, 2, NotSerialized)
        {
            ...
            ElseIf (LEqual (Arg1, One))
            {
                If (LEqual (Arg0, Zero))
                {
                    ...
                    Sleep (DLPW)
                    While (LEqual (\_SB.GGIV (0x030A0017), Zero))
                    {
                        Stall (One)
                    }
